<?php  
	...
	$food = str_replace("0"," ",$_GET['food']);
	$purchasedfood = explode(",",$food);
	...
	if(mysql_num_rows($result) > 0){
		...
		if($balance > $totalPrice){
			...
			$sql = "INSERT INTO tblPurchasedItems (cardNumber, purchasedItems, roomCategory, floor, room, status) VALUES ('". $cardSerialNumber ."','". mysql_real_escape_string($food) ."','". $roomCategory."','". $floor ."','". $room ."', 0)";
			$result = mysql_query($sql, $db) or die("0");//where 0 means unsuccessfull transaction
